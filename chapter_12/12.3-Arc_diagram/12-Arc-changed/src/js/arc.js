...
export const drawArcDiagram = (nodes, edges) => {
  ...
  const is_edge_strong = (x) => x.weight > 2;
  const get_stroke_color = (x) =>
    {
      if (is_edge_strong(x)) return "#7267eb";
      return "#364652";
    }
  svg
    .selectAll(".arc-link")
    ...
    .join("path")
      ...
      .attr("d", d => getArc(d))
      .attr("fill", "none")
      .attr("stroke", d => get_stroke_color(d))
  ...
  const people_of_house_order_x = (x) => arcNodes.filter(item => item.house === houses.find(h => h.order == x).house);
  const talkative_in_house_x = (x) => people_of_house_order_x(x).sort((a, b) => b.totalLinesNumber - a.totalLinesNumber)[0];
  const house_reprs = houses.map(h => talkative_in_house_x(h.order));
  console.log("reprs", house_reprs);
  const get_color_fill = (person) => {
    const house_order = houses.find(item => item.house === person.house).order;
    if (person.name === house_reprs[house_order - 1].name) return "#ff4a4a";
    return colorScale(person.house);
  }
  svg
    .selectAll(".arc-node")
