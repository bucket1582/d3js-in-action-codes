...
export const drawMatrix = (nodes, edges) => {
  ...
  nodes_copy = nodes_copy.sort((a, b) => b.totalLinesNumber - a.totalLinesNumber).slice(0, 5);
  ...
  nodes.forEach((charA, i) => {
    nodes.forEach((charB, j) => {
      if (charA !== charB) {
        ...
        let color;
        const charA_exists = nodes_copy.filter(item => item.name === charA.name).length > 0;
        const charB_exists = nodes_copy.filter(item => item.name === charB.name).length > 0;
