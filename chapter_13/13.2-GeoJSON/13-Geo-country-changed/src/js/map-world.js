...
export const drawWorldMap = (laureates, world) => {
  ...
  let canadaCoordinates = select("[country-name=Canada]").attr("d").split("L");
  canadaCoordinates = canadaCoordinates.map(
    coords => {
  ...
  const lineStartCoordinates = [canadaCoordinates[0] - 100, canadaCoordinates[1] - 20];
  const lineEndCoordinates = [canadaCoordinates[0] - 20, canadaCoordinates[1] - 35];
  const textCoordinates = [canadaCoordinates[0], canadaCoordinates[1] - 20];
  ...
  const canadaLaureates = world.features.filter(
    country => country.properties.name == "Canada"
