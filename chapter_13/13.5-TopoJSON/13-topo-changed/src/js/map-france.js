...
import { getCityRadius, getCityTextOpacity } from "./scales";
...
export const drawFranceMap = (laureates, france) => {
  ...
  const getCityColor = (city) => {
    return city.laureates.filter(laureate => laureate.category == "Literature").length > 0 ? "#f5ce42" : "#35a7c2";
  };
  ...
  svg
    ...
    .join("circle")
      ...
      .attr("stroke",  d => getCityColor(d));

  // Append city names
  svg
    .selectAll(".france-city-name")
    .data(cities)
    .join("text")
      .attr("class", "france-city-name")
      .attr("x", d => projection([d.longitude, d.latitude])[0])
      .attr("y", d => projection([d.longitude, d.latitude])[1])
      .attr("font-size", "1.5em")
      .attr("fill", "#f54242")
      .attr("fill-opacity", d => getCityTextOpacity(d.laureates.length, maxLaureatesPerCity))
      .text(d => d.name);
};